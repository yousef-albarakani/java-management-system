...
public class PaymentHistoryFrame extends JFrame {
    ...
    public PaymentHistoryFrame(Customer customer) {
        ...
        JLabel title = new JLabel("Payment History - " + customer.getName(), SwingConstants.CENTER);
        title.setForeground(GOLD);
        title.setFont(new Font("Georgia", Font.BOLD, 24));

        DefaultTableModel model = new DefaultTableModel(
                new String[]{"Payment ID", "Appointment ID", "Amount", "Method", "Date", "Status"}, 0
        ) {
            @Override
        ...
        ArrayList<Payment> list = paymentService.getPaymentsByCustomer(customer.getId());
