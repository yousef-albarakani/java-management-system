...
public class CustomerProfileFrame extends JFrame {
    ...
    public CustomerProfileFrame(Customer customer) {
        ...
        txtName = createField(customer.getName());

        JTextField txtUsername = createField(customer.getUsername());
        txtUsername.setEditable(false);

        txtPassword = createField(customer.getPassword());

        JTextField txtGender = createField(customer.getGender());
        txtGender.setEditable(false);

        JTextField txtAge = createField(String.valueOf(customer.getAge()));
        txtAge.setEditable(false);

        txtPhone = createField(customer.getPhone());
        txtEmail = createField(customer.getEmail());
        txtAddress = createField(customer.getAddress());

        JTextField txtNationality = createField(customer.getNationality());
    ...
    private void updateProfile() {
        ...
        for (String line : lines) {
            String[] p = line.split("\\|");
            if (p.length >= 10 && p[0].equals(customer.getId())) {
                String newLine = p[0] + "|" +
